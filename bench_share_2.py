# the attention layer from this benchmark, and other utilities such as Yarn, are from modded-nanogpt, MIT-licensed
...
class CausalSelfAttentionCG(nn.Module):
    """
    CausalSelfAttentionCG differs from CausalSelfAttentionOrig, by using explicit qkv and o layers
    instead of a shared qkvo parameter.
    this was done in an effort to eliminate confounding factors from tensor-sharing, whilst thrashing
    around to resolve cudagraphs errors. it actually seems to result in a significant performance boost too.
    """
    def __init__(self, dim: int, head_dim: int, num_heads: int):
        super().__init__()
        self.num_heads = num_heads
